...
export class InputDirective implements AfterViewInit, OnDestroy, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      (!iconRightChange?.firstChange && iconRightChange?.currentValue != iconRightChange?.previousValue) ||
      (!iconLeftChange?.firstChange && iconLeftChange?.currentValue != iconLeftChange?.previousValue)
    ) {
      this.updateIcon();
    }
  ...
  addDescription(): void {
    if (this.helpDesc) {
      ...
      this.rendered.appendChild(parentCurrentElement.parentNode, this.paragraphHelpDesc);
  ...
  addErrorDescription(message = ''): void {
    ...
    if (message) {
      this.paragraphErrorDesc
        ? this.rendered.removeChild(currentElement.parentNode.parentNode, this.paragraphErrorDesc)
        : null;
      ...
      this.rendered.appendChild(parentCurrentElement.parentNode, this.paragraphErrorDesc);
  ...
  removeInputLabel(): void {
    if (this.labelInputEl) {
      const currentElement = this.el.nativeElement;
      this.rendered.removeChild(currentElement.parentNode.parentNode, this.labelInputEl);
    }
  }
  /**
  ...
  addClass(error: boolean): void {
    if (error) {
      ...
      this.iconElementLeft ? this.rendered.addClass(this.iconElementLeft, 'text-error-200') : null;
      this.iconElementRight ? this.rendered.addClass(this.iconElementRight, 'text-error-200') : null;
    } else {
      this.rendered.removeClass(this.el.nativeElement, 'input-error');
      this.rendered.addClass(this.el.nativeElement, 'input-default');
      this.iconElementLeft ? this.rendered.removeClass(this.iconElementLeft, 'text-error-200') : null;
      this.iconElementRight ? this.rendered.removeClass(this.iconElementRight, 'text-error-200') : null;
    }
  }
